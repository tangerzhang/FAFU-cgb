...
while(my $file = glob "$opt_q/Lib_R1_*.fastq"){
	my $id     = $1 if($file=~/Lib_R1_(\d+).fastq/);
	my $r1     = $opt_q."/Lib_R1_".$id.".fastq";
	my $r2     = $opt_q."/Lib_R2_".$id.".fastq";
	...
	close $out;
#        system("qsub -S /bin/bash -cwd -pe mpi 24 -q all.q scripts/$outfile");
	}
